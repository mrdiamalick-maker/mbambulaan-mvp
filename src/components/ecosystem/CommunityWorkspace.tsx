...
// Restylé en D9 (Lot 7, étape 2/4). CommandButton (ancien,
// @/components/ui/CommandButton) remplacé par Button+run(). Classes
// utilitaires sarcelle (btn-primary/btn-secondary/data-chip/link-action)
// remplacées par les composants shadcn déjà en place.
...
export function CommunityWorkspace() {
  ...
  return (
    <div className="space-y-6">
      <Card className="overflow-hidden">
        <div className="grid lg:grid-cols-[1.2fr_.8fr]">
          <div className="p-6 lg:p-8">
            ...
            <Button variant="outline" className="mt-6" asChild><Link href="/app/initiatives">Ouvrir les programmes &amp; financements <ArrowRight size={16} /></Link></Button>
          </div>
          <div className="grid grid-cols-3 border-t bg-muted/40 lg:border-l lg:border-t-0">
            {[
              [state.communityPosts.length, "contributions"],
              [transformed, "qualifiées en situation"],
              [state.territories.length, "territoires reliés"]
            ].map(([value, label]) => <div key={String(label)} className="flex flex-col justify-center border-r p-4 last:border-r-0"><p className="text-2xl font-bold">{value}</p><p className="mt-2 text-[11px] font-bold leading-4 text-muted-foreground">{label}</p></div>)}
          </div>
        </div>
      </Card>
      ...
      <section className="grid gap-5 xl:grid-cols-[.7fr_1.3fr]">
        <Card className="h-fit">
          <CardContent className="p-5">
            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Faire remonter une information utile</p>
        ...
        <div className="space-y-4">
          <Card><CardContent className="grid gap-3 p-4 sm:grid-cols-[1fr_auto]"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un besoin, une capacité, un apprentissage…" className="rounded-md border bg-background p-2.5 text-sm outline-none focus:border-primary" /><div className="inline-flex items-center gap-2 text-xs font-bold text-muted-foreground"><Filter size={15} /> {posts.length} contribution(s)</div></CardContent></Card>
          {posts.map((post) => {
            ...
            return (
              <Card key={post.id}>
                <CardContent className="p-5">
                  ...
                  <h2 className="mt-4 text-lg font-semibold">{post.title}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{post.body}</p>
                  <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t pt-4">
                    ...
                    {post.status !== "transforme" ? (
                      <Button size="sm" variant="outline" disabled={converting === post.id} onClick={() => void convert(post.id)}><Sparkles size={15} /> {converting === post.id ? "…" : "Qualifier en situation"}</Button>
                  ...
                  {post.comments.map((comment) => <div key={comment.id} className="mt-3 flex gap-2 rounded-md bg-muted p-3 text-xs leading-5"><MessageCircle size={15} className="shrink-0 text-[#1d4468]" /><span>{comment.body}</span></div>)}
                </CardContent>
              </Card>
            );
          })}
      ...
      <p className="text-xs text-muted-foreground">Données de démonstration. Une contribution n’est diffusée au-delà de son périmètre qu’après qualification et selon les droits du mandat.</p>
    </div>
  );
}
