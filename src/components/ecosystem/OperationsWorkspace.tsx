...
// remplacés par Button+run() et le TrustBadge partagé. mission-strip/
// ops-filter (classes utilitaires sarcelle) remplacées par le dégradé
// marine déjà utilisé pour les héros D9.
  ...
  return (
    <div className="space-y-8">
      <Card className="overflow-hidden border-none bg-sidebar text-sidebar-foreground shadow-lg">
        <CardContent className="p-5 lg:p-6">
          <div className="grid gap-5 lg:grid-cols-[1fr_300px] lg:items-end">
            <div>
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-primary"><Radio size={14} /> Console des opérations</div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Card><CardContent className="p-4"><ShipWheel size={18} className="text-[#1d4468]" /><p className="mt-3 text-2xl font-bold">{state.vessels.length}</p><p className="text-xs text-muted-foreground">Pirogues suivies</p><p className="mt-1 text-[11px] text-muted-foreground">Périmètre de démonstration, immatriculations fictives</p></CardContent></Card>
        <Card><CardContent className="p-4"><TimerReset size={18} className="text-[#c68a2c]" /><p className="mt-3 text-2xl font-bold">{state.trips.filter((item) => item.status !== "debarquee").length}</p><p className="text-xs text-muted-foreground">Sorties en cours</p><p className="mt-1 text-[11px] text-muted-foreground">Retours attendus ou annoncés aujourd’hui</p></CardContent></Card>
        <Card><CardContent className="p-4"><Scale size={18} className="text-[#1d8a5f]" /><p className="mt-3 text-2xl font-bold">{(totalKg / 1000).toFixed(2)} t</p><p className="text-xs text-muted-foreground">Volume pesé</p><p className="mt-1 text-[11px] text-muted-foreground">Pesées vérifiées dans le jeu de démonstration</p></CardContent></Card>
        <Card><CardContent className="p-4"><Fish size={18} className="text-[#1d4468]" /><p className="mt-3 text-2xl font-bold">{state.lots.filter((item) => item.status === "disponible").length}</p><p className="text-xs text-muted-foreground">Lots disponibles</p><p className="mt-1 text-[11px] text-muted-foreground">Reliés à une sortie et un débarquement</p></CardContent></Card>
      </section>
          ...
          <div className="grid gap-px bg-border sm:grid-cols-4">
            {[
              ...
              <div key={String(label)} className="bg-card p-4">
                <span className={`grid size-7 place-items-center rounded-full text-xs font-bold ${done ? "bg-[#1d8a5f] text-white" : "bg-muted text-muted-foreground"}`}>{index + 1}</span>
                <p className="mt-3 text-sm font-semibold">{String(label)}</p>
                <p className="mt-1 text-xs text-muted-foreground">{done ? "Traçable" : "À réaliser"}</p>
              </div>
            ))}
          </div>
          <div className="space-y-5 p-5">
            <div className="grid gap-4 sm:grid-cols-2">
              <dl className="space-y-2 text-sm">
                <div><dt className="text-muted-foreground">Immatriculation</dt><dd className="font-semibold">{vessel?.registration}</dd></div>
                <div><dt className="text-muted-foreground">Zone déclarée</dt><dd className="font-semibold">{primaryTrip.zone}</dd></div>
                <div><dt className="text-muted-foreground">Retour prévu</dt><dd className="font-semibold">{new Intl.DateTimeFormat("fr-FR", { dateStyle: "medium", timeStyle: "short" }).format(new Date(primaryTrip.expectedReturnAt))}</dd></div>
              </dl>
              <div className="rounded-md border-l-4 border-l-[#1d8a5f] bg-[#1d8a5f]/8 p-4">
                <p className="text-xs font-bold uppercase text-[#146144]">Ce qui est attendu maintenant</p>
                <p className="mt-2 text-sm font-semibold">{action?.label ?? (landing?.status === "arrive" ? "Confirmer la pesée" : landing?.status === "pese" ? "Créer les lots" : "Consulter les lots disponibles")}</p>
          ...
          <CardContent className="p-5">
            ...
            <div className="mt-5 space-y-3">
              {landing?.catches.map((catchLine) => {
                ...
                return (
                  <div key={catchLine.id} className="flex items-center justify-between border-b pb-3 text-sm last:border-0">
            ...
            <div className="mt-5 rounded-md bg-[#c68a2c]/10 p-4">
