...
>
> **Établie au Lot 6, mise à jour au Lot 7, puis lors de la revue éditoriale Produit (2026-08-12).** Le Lot 7 a fermé quatre des limites listées à l'issue du Lot 6 (rôles hors périmètre, homogénéisation D9, D10, audit mobile). La revue éditoriale du 2026-08-12 (livrable 2/5, traitement du wording du score) a mis au jour une 3e exception au design homogène du §2.8, non détectée jusque-là — voir ci-dessous. Ce document reflète cet état — les versions précédentes restent consultables dans l'historique git de ce fichier.

## Statut global

**Le Produit satisfait les 15 critères §21 sur le parcours canonique**, et les 4 limites assumées à l'issue du Lot 6 sont closes. Il reste trois exceptions **délibérées et bornées** au design homogène (deux assumées dès le Lot 7, une 3e mise au jour le 2026-08-12 et explicitement différée), et une frontière de périmètre V1 inchangée (WhatsApp/SMS réels) — détaillées en §3. Build vert (lint/typecheck/test/build/test:e2e) à chaque étape des Lots 0 à 7 et de chaque livrable de la revue éditoriale, sans exception.
...
| 8 | Le design change d'une page à l'autre | ✅ Absent, **3 exceptions délibérées et bornées** | **Fermé au Lot 7** (étape 2/4, 5 commits, 8 pages migrées + 5 panneaux co-rendus), **puis 3e exception documentée le 2026-08-12**. (a) le poste de commande sombre de `ProfessionalAtlasWorkspace.tsx` (carte, positions territoriales) — retouché uniquement pour son code de couleur aurait fait courir un risque de régression sur le code de positionnement qui portait le bug des 18 territoires corrigé avant le Lot 5 ; (b) `/app/situations`, décidé au Lot 3 pour rester sur son propre système au bénéfice des rôles task-first. (a) et (b) documentées dans le code (commentaires) et dans les commits `5ab4601`/historique Lot 3. **(c) `CoordinationWorkspace.tsx` (page `/app/coordination`, vue liste)** — **Constat** : n'a jamais été migré au design D9 ; porte encore intégralement la palette sarcelle pré-D9 (`#08758a`, `#075568`, `#e1e9e9`…) et plusieurs enums bruts affichés sans libellé (`need.status`, `need.intent`, `capacity.type`). Contrairement à (a)/(b), ce n'était pas une décision assumée au moment du Lot 7 : l'écart avait échappé à la fois à l'homogénéisation D9 (étape 2/4) et à l'audit mobile (étape 4/4), qui portaient sur d'autres critères. **Source** : mis au jour le 2026-08-12 en traitant l'affichage du score d'`Opportunity` (livrable 2/5 de la revue éditoriale Produit) — seul le texte du score y a été corrigé à cette occasion (texte-seul, motif de micro-label déjà présent dans le fichier, aucune classe/couleur nouvelle), pas le reste de la page. **Recommandation** : restylisation D9 complète de `CoordinationWorkspace.tsx` à traiter comme un chantier structurel séparé, après la revue éditoriale en cours — décision CEO du 2026-08-12 de différer plutôt que de l'absorber dans ce lot éditorial. |
...
**Aucune limite du Lot 6 ne reste ouverte sans explication.** Les trois exceptions du §2.8 sont les seuls écarts restants au design homogène. Les deux premières ((a), (b)) sont bornées et justifiées par un risque de régression réel plutôt que par un manque de temps ; la 3e ((c), `CoordinationWorkspace.tsx`) est d'une nature différente — un écart non détecté avant le 2026-08-12, pas une décision prise au moment du Lot 7 — mais elle est désormais documentée, bornée à un fichier identifié, et sa correction (restylisation D9) est explicitement planifiée comme un chantier séparé plutôt que laissée sans suite.
