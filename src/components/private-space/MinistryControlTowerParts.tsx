...
export function MinistryMap({ viewMode, quays, pirogues, alerts, landings, selectedId, selectedKind, onSelectQuay, onSelectPirogue }: {
  ...
  return <div className="relative min-h-[43rem] overflow-hidden rounded-[1.5rem] border border-slate-300 bg-[#061827] text-white shadow-[0_32px_90px_rgba(15,23,42,0.28)]">
    <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
      <defs>
        <linearGradient id="premiumOcean" x1="0" x2="1" y1="0" y2="1">
          <stop offset="0%" stopColor="#061827" />
          <stop offset="48%" stopColor="#0b3348" />
          <stop offset="100%" stopColor="#115e63" />
        </linearGradient>
        <linearGradient id="premiumLand" x1="0" x2="1" y1="0" y2="1">
          <stop offset="0%" stopColor="#e7d7ad" />
          <stop offset="52%" stopColor="#b6a36d" />
          <stop offset="100%" stopColor="#58725c" />
        </linearGradient>
        <filter id="coastShadow">
          <feDropShadow dx="0" dy="1.2" stdDeviation="1.4" floodColor="#020617" floodOpacity="0.45" />
        </filter>
      </defs>
      <rect width="100" height="100" fill="url(#premiumOcean)" />
      <path d="M0 80 C18 75 27 64 39 56 C54 45 70 27 100 13 L100 100 L0 100 Z" fill="#0e7490" opacity="0.12" />
      <path d="M7 20 C21 28 31 36 39 47 C51 62 66 72 92 84" fill="none" stroke="#e2e8f0" strokeWidth="0.18" strokeDasharray="1.5 2" opacity="0.45" />
      <path d="M43 2 C49 10 46 21 50 30 C56 43 50 50 57 60 C64 71 55 81 65 98 L100 98 L100 2 Z" fill="url(#premiumLand)" opacity="0.99" />
      <path d="M43 2 C49 10 46 21 50 30 C56 43 50 50 57 60 C64 71 55 81 65 98" fill="none" stroke="#f8fafc" strokeWidth="1.15" filter="url(#coastShadow)" />
      <path d="M44 3 C50 12 47 21 52 30 C57 40 52 50 59 61 C64 69 58 78 66 96" fill="none" stroke="#0f172a" strokeWidth="0.26" strokeDasharray="1.1 1.4" opacity="0.48" />
      {viewMode === "pirogues" ? pirogues.map((pirogue) => {
        const quay = quays.find((item) => item.id === pirogue.quayId);
        if (!quay) return null;
        return <path key={`track-${pirogue.id}`} d={`M ${quay.x} ${quay.y} C ${quay.x - 6} ${quay.y - 2}, ${pirogue.x + 4} ${pirogue.y + 1}, ${pirogue.x} ${pirogue.y}`} fill="none" stroke="#dbeafe" strokeWidth="0.42" strokeDasharray="1.1 1.7" opacity="0.82" />;
      }) : null}
      <text x="73" y="13" fill="#1f2937" fontSize="3.2" fontWeight="800">SÉNÉGAL</text>
      <text x="6" y="18" fill="#dbeafe" fontSize="2.8" fontWeight="800">OCÉAN ATLANTIQUE</text>
      <text x="52" y="9" fill="#1f2937" fontSize="2.3" fontWeight="800">Saint-Louis</text>
      <text x="59" y="36" fill="#1f2937" fontSize="2.3" fontWeight="800">Louga</text>
      <text x="61" y="49" fill="#1f2937" fontSize="2.3" fontWeight="800">Thiès</text>
      <text x="62" y="56" fill="#1f2937" fontSize="2.3" fontWeight="800">Dakar</text>
      <text x="70" y="88" fill="#1f2937" fontSize="2.3" fontWeight="800">Casamance</text>
    </svg>

    <div className="absolute inset-0 bg-[linear-gradient(rgba(226,232,240,0.07)_1px,transparent_1px),linear-gradient(90deg,rgba(226,232,240,0.07)_1px,transparent_1px)] bg-[size:56px_56px] opacity-35" />

    <div className="absolute left-4 top-4 z-40 max-w-[19rem] rounded-xl border border-white/15 bg-slate-950/60 p-4 backdrop-blur-md">
      ...
      <p className="mt-2 text-xs font-semibold leading-5 text-slate-300">Les quais sont ancrés au littoral. Les pirogues sont positionnées en mer, légèrement au large de leur quai.</p>
    </div>

    <div className="absolute right-4 top-4 z-40 rounded-xl border border-white/15 bg-slate-950/45 p-3 text-xs font-black backdrop-blur-md">
    ...
    {alerts.map((alert) => {
      ...
      if (!quay) return null;
      return <span key={alert.id} className={`absolute z-10 -translate-x-1/2 -translate-y-1/2 rounded-full border ${alert.level === "urgent" ? "border-rose-200 bg-rose-500/20 shadow-[0_0_34px_rgba(244,63,94,.44)]" : "border-amber-200 bg-amber-400/18 shadow-[0_0_28px_rgba(245,158,11,.32)]"}`} style={{ left: `${quay.x}%`, top: `${quay.y}%`, width: 72, height: 72 }} title={alert.title} />;
    })}

    {quays.map((quay) => {
      const quayLandings = landings.filter((landing) => landing.quayId === quay.id);
      return <button key={quay.id} onClick={() => onSelectQuay(quay.id)} className={`absolute z-40 -translate-x-1/2 -translate-y-1/2 rounded-lg border p-1.5 transition ${viewMode === "pirogues" ? "opacity-70" : "opacity-100"} ${selectedKind === "quay" && selectedId === quay.id ? "border-white bg-white shadow-[0_0_0_8px_rgba(226,232,240,.2),0_0_32px_rgba(255,255,255,.62)]" : quay.level === "urgent" ? "border-white bg-rose-500" : quay.level === "surveillance" ? "border-white bg-amber-400" : "border-white bg-emerald-500"}`} style={{ left: `${quay.x}%`, top: `${quay.y}%` }} aria-label={`Sélectionner ${quay.name}`}>
        <span className="block h-3 w-3 rounded-sm bg-white" />
        <span className="absolute left-6 top-1/2 -translate-y-1/2 whitespace-nowrap rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-black text-slate-950 shadow-sm">{quay.name}</span>
        {viewMode === "quays" ? <span className="absolute left-6 top-8 whitespace-nowrap rounded-lg border border-white/15 bg-slate-950/85 px-2 py-1 text-[10px] font-black text-white shadow-sm">{quayLandings.length} débarq. · {sumLandingVolume(quayLandings)} t</span> : null}
      </button>;
    })}

    {viewMode === "pirogues" ? pirogues.map((pirogue) => {
      const landing = landings.find((item) => item.pirogueIds.includes(pirogue.id));
      return <button key={pirogue.id} onClick={() => onSelectPirogue(pirogue.id)} className={`absolute z-50 -translate-x-1/2 -translate-y-1/2 rounded-lg border px-2.5 py-1.5 text-[10px] font-black shadow-sm transition ${selectedKind === "pirogue" && selectedId === pirogue.id ? "border-white bg-white text-slate-950 shadow-[0_0_0_5px_rgba(255,255,255,.2)]" : pirogue.level === "urgent" ? "border-white bg-rose-100 text-rose-950 hover:bg-white" : "border-white/70 bg-slate-100 text-slate-950 hover:bg-white"}`} style={{ left: `${pirogue.x}%`, top: `${pirogue.y}%` }} title={pirogue.registration}>
    ...
    <div className="absolute bottom-4 left-4 right-4 z-50 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/15 bg-slate-950/58 p-3 text-xs font-black backdrop-blur-md">
      <div className="flex flex-wrap gap-2">
        <span className="rounded-lg bg-white px-3 py-2 text-slate-950">Quai sur le littoral</span>
        {viewMode === "pirogues" ? <span className="rounded-lg bg-slate-200 px-3 py-2 text-slate-950">Pirogue immatriculée</span> : <span className="rounded-lg bg-slate-200 px-3 py-2 text-slate-950">Débarquements par quai</span>}
      ...
      <span className="rounded-lg border border-white/20 px-3 py-2 text-slate-200">Échelle visuelle simulée · Littoral nord-sud</span>
    </div>
  </div>;
}
