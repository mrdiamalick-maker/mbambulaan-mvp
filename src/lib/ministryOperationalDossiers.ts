...
export type DossierType = "Vérification" | "Incident" | "Financement" | "Rapport" | "Note";
...
export type DossierAction = "request-verification" | "prepare-whatsapp" | "follow-verification" | "deposit-constat" | "validate-constat" | "generate-report" | "open-atlas" | "open-funding" | "open-pilotage" | "open-note" | "none";
...
export function buildOperationalDossiers({ verificationTasks, signals, fundingDossiers, opportunities, reports, decisions }: {
  ...
  const dossiers: DossierOperationnel[] = [
    buildKayarDossier(kayarTask, kayarReport),
    {
      id: "INC-2026-0081", type: "Incident", linkedObject: "Retour de pirogue non confirmé", linkedObjectType: "Zone", sourceId: "incident-2", quayId: "saint-louis", territory: "Saint-Louis",
    ...
    {
      id: "RAP-2026-0022", type: "Rapport", linkedObject: "Situation maritime de Joal-Fadiouth", linkedObjectType: "Zone", sourceId: "joal", quayId: "joal", territory: "Thiès",
  ...
  signals.slice(0, 2).forEach((signal, index) => dossiers.push({
    id: `INC-2026-${String(90 + index).padStart(4, "0")}`, type: "Incident", linkedObject: signal.title, linkedObjectType: "Zone", sourceId: signal.id, territory: signal.scope,
    workStatus: signal.status === "Clôturé" ? "Terminé" : signal.status === "Signalé" ? "Nouveau" : signal.status === "En traitement" ? "En attente" : "À traiter",
    businessStatus: signal.status, originChannel: "WhatsApp", transitChannel: "Agent territorial", currentOwner: signal.receivingCell,
  ...
  fundingDossiers.forEach((dossier, index) => dossiers.push({
    id: `FIN-2026-${String(31 + index).padStart(4, "0")}`, type: "Financement", linkedObject: dossier.title, linkedObjectType: "Besoin", sourceId: dossier.id, territory: dossier.targetPartner,
  ...
  if (!fundingDossiers.length && opportunities[0]) {
    ...
    dossiers.push({
      id: "FIN-2026-0031", type: "Financement", linkedObject: opportunity.title, linkedObjectType: "Besoin", sourceId: opportunity.id, territory: opportunity.territory,
  ...
  decisions.slice(0, 2).forEach((decision, index) => dossiers.push({
    id: `NOT-2026-${String(7 + index).padStart(4, "0")}`, type: "Note", linkedObject: decision.noteTitle, linkedObjectType: "Zone", sourceId: decision.id, territory: decision.source,
...
function buildKayarDossier(task?: VerificationTask, report?: ZoneReportRecord): DossierOperationnel {
  const status = task?.status;
  const reportReady = Boolean(report);
  const workStatus: DossierWorkStatus = reportReady ? "Terminé" : !task ? "À traiter" : ["Message préparé", "Assignée", "En cours"].includes(status || "") ? "En attente" : status === "Constat déposé" || status === "Vérifiée" ? "À traiter" : "À traiter";
  const action: DossierAction = reportReady ? "none" : !task ? "request-verification" : status === "Demandée" ? "prepare-whatsapp" : status === "Message préparé" ? "follow-verification" : ["Assignée", "En cours"].includes(status || "") ? "deposit-constat" : status === "Constat déposé" ? "validate-constat" : "generate-report";
  const nextAction = reportReady ? "Relire le rapport puis décider de son archivage" : !task ? "Demander une vérification terrain" : status === "Demandée" ? "Préparer le message WhatsApp" : status === "Message préparé" ? "Suivre la vérification" : ["Assignée", "En cours"].includes(status || "") ? "Déposer le constat reçu" : status === "Constat déposé" ? "Valider le constat" : "Générer le rapport de zone";
  ...
  return {
    id: "VER-2026-0142", type: "Vérification", linkedObject: "Écart de pesée au quai de Kayar", linkedObjectType: "Quai", sourceId: "kayar", quayId: "kayar", territory: "Thiès",
    workStatus, businessStatus: reportReady ? "Rapport disponible" : task?.status || "Vérification à demander", originChannel: "Poste de quai", transitChannel: task ? "WhatsApp" : undefined,
