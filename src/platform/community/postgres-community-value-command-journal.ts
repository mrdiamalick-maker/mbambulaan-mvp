...
import type { CommunityCommand } from "./community-value-loop-runtime";

export interface PersistedCommunityCommand {
  commandId: string;
...
export class PostgresCommunityValueCommandJournal {
  constructor(private readonly executor: () => SqlExecutor) {}

  async append(entry: PersistedCommunityCommand) {
    await this.executor().query(
      `INSERT INTO mbambulaan.community_value_command_journal
  ...
  async list() {
    const result = await this.executor().query<{
      ...
      territory_id: string;
      payload: CommunityCommand;
      occurred_at: string;
    }>(
      `SELECT command_id, actor_identity_id, territory_id, payload, occurred_at
       FROM mbambulaan.community_value_command_journal
    ...
    } satisfies PersistedCommunityCommand));
  }
}
