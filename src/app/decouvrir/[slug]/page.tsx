  ...
  return (
    <main className="min-h-screen bg-[#f3f7f6]">
      ...
      <section className="mx-auto max-w-3xl px-5 py-12 md:px-10 md:py-16">
        ...
        <div className="mt-8 flex items-center gap-2 rounded-xl border border-[#d9e3e3] bg-white px-4 py-3 text-xs font-semibold text-[#60737a]">
          <BadgeCheck size={15} className="shrink-0 text-[#118f83]" /> {item.verification}
        </div>
        ...
        {item.cta && (
          <div className="mt-8 rounded-2xl bg-[#eaf5f1] p-6">
            ...
            <Link href={item.cta.href} className="mt-3 inline-flex items-center gap-2 text-base font-black text-[#0a6d68]">{item.cta.label} <ArrowRight size={16} /></Link>
          </div>
        )}
        ...
        {territory && (
          <Link href={`/atlas/${territory.slug}`} className="mt-6 flex items-center gap-3 rounded-2xl border border-[#d9e3e3] bg-white p-5 transition hover:border-[#8fc3bd]">
            <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#e5f7f3] text-[#075568]"><MapPinned size={18} /></span>
            <span><strong className="block text-sm font-bold text-[#102e37]">Voir le territoire : {territory.name}</strong><span className="text-xs text-[#718489]">Portrait, activités et services documentés dans l’Atlas Mbàmbulaan.</span></span>
          </Link>
        )}
      </section>
      ...
      {related.length > 0 && (
        <section className="border-t border-[#d9e3e3] bg-white px-5 py-14 md:px-10 md:py-18">
          <div className="mx-auto max-w-[1500px]">
            <p className="label">À lire aussi</p>
            <h2 className="mt-3 text-2xl font-[740] tracking-[-.035em] text-[#102e37] md:text-3xl">{item.domain}</h2>
            <div className="mt-8 grid gap-4 lg:grid-cols-3">
              {related.map((news) => (
                <Link key={news.id} href={`/decouvrir/${news.id}`} className="surface group flex min-h-56 flex-col p-5 transition hover:-translate-y-0.5 hover:border-[#8fc3bd]">
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.11em] text-[#118f83]"><BookOpenText size={14} />{news.category}</div>
                  <h3 className="mt-4 text-lg font-bold tracking-[-.025em] text-[#102e37]">{news.title}</h3>
                  <p className="mt-3 text-sm leading-6 text-[#667b81]">{news.excerpt}</p>
