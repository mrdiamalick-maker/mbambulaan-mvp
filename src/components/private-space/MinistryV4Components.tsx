...
export function FiliereNeedsView({ needs, opportunities, artifacts, qualifiedNeedIds = [], fundingDossierNeedIds = [], partnerPreparedNeedIds = [], onOpenWorkflow }: { needs: CommunityNeed[]; opportunities: FundingOpportunity[]; artifacts: GeneratedArtifact[]; qualifiedNeedIds?: string[]; fundingDossierNeedIds?: string[]; partnerPreparedNeedIds?: string[]; onOpenWorkflow: (kind: "qualification" | "funding" | "partner", context: WorkflowContext) => void }) {
  ...
  return <div className="grid min-w-0 gap-2 bg-[var(--mb-neutral-100)] p-2 xl:grid-cols-[minmax(0,1fr)_330px]">
    ...
    <aside className="grid content-start gap-2">{selected && context ? <section className="border border-[var(--mb-neutral-200)] bg-white"><header className="bg-[var(--mb-navy-900)] px-3 py-3 text-white"><p className="font-mono text-[8px] uppercase text-[var(--mb-ocean-400)]">Partenaires & preuves</p><h3 className="mt-1 text-[13px] font-semibold">{selected.opportunity?.title || selected.need.need}</h3><p className="mt-1 text-[9px] text-white/55">{selected.need.place} · {selected.status}</p><div className="mt-2"><DataTrustBadge level={selected.need.trustLevel} compact /></div></header><dl className="divide-y divide-[var(--mb-neutral-100)] px-3"><Fact label="Partenaire compatible" value={selected.opportunity?.compatibleFunder || "À identifier"} /><Fact label="Compatibilité" value={selected.opportunity ? `${selected.opportunity.compatibilityScore}%` : "À calculer"} /><Fact label="Impact attendu" value={selected.opportunity?.expectedImpact || "À documenter"} /><Fact label="Preuve disponible" value={artifacts.some((artifact) => artifact.scope.includes(selected.need.place)) ? "Document enregistré" : "Preuve terrain manquante"} /></dl><div className="grid gap-2 border-t border-[var(--mb-neutral-100)] p-3"><p className="text-[9px] leading-4 text-[var(--mb-neutral-600)]">{actionHelper(selected.status)}</p><NeedActions row={selected} context={context} partnerPrepared={partnerPreparedNeedIds.includes(selected.need.id)} onOpenWorkflow={onOpenWorkflow} /></div></section> : null}
...
function NeedActions({ row, context, partnerPrepared, onOpenWorkflow }: { row: NeedViewRow; context: WorkflowContext; partnerPrepared: boolean; onOpenWorkflow: (kind: "qualification" | "funding" | "partner", context: WorkflowContext) => void }) {
  ...
  if (row.status === "Dossier constitué" && partnerPrepared) return <span className="border-l-2 border-[var(--mb-ocean-600)] bg-[var(--mb-foam)] px-3 py-2 text-[9px] font-semibold text-[var(--mb-navy-900)]">Sollicitation préparée · prochaine étape : noter la réponse du partenaire</span>;
  if (row.status === "Dossier constitué") return <button onClick={() => onOpenWorkflow("partner", context)} className={primaryButton}>Solliciter un partenaire</button>;
