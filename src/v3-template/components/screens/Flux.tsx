...
// PD.4 — Flux entrant connecté au domaine réel (IncomingMessage). Seul
// point où cet écran touche le domaine réel Mbàmbulaan ; voir l'en-tête
// de lib/flux-bridge.ts. Structure visuelle du gabarit gelé préservée à
// l'identique (bandeau de paliers, liste + détail deux colonnes,
// "Ce que le système a pu rattacher"/"Ce qui manque", qualification) —
// seuls les paliers (3 réels au lieu de 4 fixture, mandat §1 : "do not
// invent new workflow stages") et les actions (2 commandes réelles au
      ...
      <div style={{ display: "grid", gridTemplateColumns: "400px 1fr", gap: 0, border: "1px solid rgba(11,26,42,.12)", alignItems: "start" }}>
        <div style={{ background: "#FFFFFF", borderRight: "1px solid rgba(11,26,42,.12)", alignSelf: "stretch" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 16px", borderBottom: "1px solid rgba(11,26,42,.09)", background: "rgba(11,26,42,.03)" }}>
            <span style={{ fontSize: 11.5, color: "rgba(11,26,42,.6)", flex: 1 }}>{filtered.length} élément{filtered.length > 1 ? "s" : ""} · {stages.find((s2) => s2.key === stage)?.label.toLowerCase()}</span>
            <span style={{ fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", color: "rgba(11,26,42,.42)" }}>Plus ancien en premier</span>
          </div>
          {filtered.map((x) => (
            <button
              key={x.id}
              onClick={() => patch({ dossOpen: x.id, dossChoice: null })}
              style={{
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 7 }}>
                <span style={{ fontSize: 10.5, letterSpacing: ".06em", textTransform: "uppercase", color: x.channelColor }}>{x.channelLabel}</span>
                <span style={{ flex: 1 }} />
                <span style={{ fontFamily: V3_FONT_MONO, fontSize: 10.5, color: "rgba(11,26,42,.6)" }}>{x.ageLabel}</span>
              </div>
          ...
          {filtered.length === 0 && (
            <div style={{ padding: "30px 18px", fontSize: 12.5, lineHeight: 1.55, color: "rgba(11,26,42,.6)" }}>
              Aucun élément réel du Demo World n’est actuellement à ce palier.
            </div>
          )}
        </div>
            ...
            <>
              <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid rgba(11,26,42,.1)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 11, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 10, letterSpacing: ".12em", textTransform: "uppercase", color: "#B6522F" }}>{f.channelLabel}</span>
                  <span style={{ fontSize: 11.5, color: "rgba(11,26,42,.55)" }}>{f.from} · {f.territoryLabel} · reçu {f.receivedLabel}</span>
                </div>
                <h2 style={{ fontFamily: V3_FONT_SERIF, fontWeight: 400, fontSize: 25, lineHeight: 1.2, margin: "0 0 12px", maxWidth: "36ch" }}>{f.title}</h2>
                <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.6, color: "rgba(11,26,42,.8)", maxWidth: "78ch", padding: "13px 16px", background: "#F7F3E9", borderLeft: "2px solid rgba(11,26,42,.2)" }}>{f.body}</p>
              </div>
                  ...
                  {f.matched.length > 0 ? (
                    f.matched.map((m, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid rgba(11,26,42,.06)" }}>
                        ...
                        <span style={{ fontSize: 12.5, color: "rgba(11,26,42,.7)" }}>{m.value}</span>
                      </div>
                    ))
                  ) : (
                    <div style={{ fontSize: 12, color: "rgba(11,26,42,.5)" }}>Rien n’a pu être rattaché automatiquement.</div>
                  )}
                </div>
                <div style={{ background: "#FFFFFF", padding: "16px 22px" }}>
