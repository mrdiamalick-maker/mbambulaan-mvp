...
function DataMaturity() {
  ...
  return <section className="border-y border-[var(--mb-neutral-200)] bg-[var(--mb-foam)]"><div className="mx-auto max-w-[88rem] px-5 py-16 sm:px-8 lg:px-12"><SectionIntroduction eyebrow="D’où viennent les données" title="Une confiance construite par étapes" text="Aucun signal communautaire n’alimente une décision nationale avant qualification locale ou régionale." /><div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]"><div className="grid gap-px bg-[var(--mb-neutral-200)] sm:grid-cols-2">{levels.map(([label, helper, tone]) => <div key={label} className="bg-white p-4"><div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${tone}`} /><strong className="text-[14px] text-[var(--mb-navy-900)]">{label}</strong></div><p className="mt-2 text-[14px] leading-6 text-[var(--mb-neutral-600)]">{helper}</p></div>)}</div><ol className="grid border-t border-[var(--mb-neutral-200)]">{path.map(([stage, text], index) => <li key={stage} className="grid grid-cols-[3rem_6rem_minmax(0,1fr)] items-center border-b border-[var(--mb-neutral-200)] py-3"><span className="font-mono text-[10px] text-[var(--mb-ocean-600)]">0{index + 1}</span><strong className="text-[13px] text-[var(--mb-navy-900)]">{stage}</strong><span className="text-[14px] text-[var(--mb-neutral-600)]">{text}</span></li>)}</ol></div></div></section>;
}
