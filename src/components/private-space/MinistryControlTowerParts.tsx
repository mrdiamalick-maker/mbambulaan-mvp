...
export function MapCanvas({ mode, layers, onToggleLayer, quays, pirogues, landings, alerts, incidents = [], selection, onSelectQuay, onSelectPirogue }: {
  ...
  return <div className="relative h-full min-h-[430px] overflow-hidden bg-[#a9c8d2]">
    ...
    <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><path d="M54 0 C49 11 55 20 51 31 C47 43 55 50 51 61 C47 72 57 83 52 100 L0 100 L0 0 Z" fill="#ede7d8" /><path d="M54 0 C49 11 55 20 51 31 C47 43 55 50 51 61 C47 72 57 83 52 100" fill="none" stroke="#fff" strokeWidth="1" /><path d="M55 0 C51 11 57 20 53 31 C49 43 57 50 53 61 C49 72 59 83 54 100" fill="none" stroke="#2a6f8e" strokeDasharray="1 1.4" strokeWidth=".2" opacity=".7" /><path d="M60 0 C55 13 63 25 58 38 C53 51 62 66 59 100" fill="none" stroke="#d8eef3" strokeWidth=".18" opacity=".42" /><path d="M68 0 C62 15 71 30 66 47 C61 64 70 80 68 100" fill="none" stroke="#d8eef3" strokeWidth=".15" opacity=".34" /><path d="M78 0 C72 18 81 34 76 53 C71 70 82 86 80 100" fill="none" stroke="#d8eef3" strokeWidth=".12" opacity=".28" />{mode === "pirogues" && layers.pirogues ? pirogues.map((boat) => { const quay = quays.find((item) => item.id === boat.quayId); return quay ? <path key={boat.id} d={`M ${quay.x} ${quay.y} Q ${boat.x + 5} ${boat.y - 6} ${boat.x} ${boat.y}`} fill="none" stroke="#12314f" strokeDasharray="1 1.2" strokeWidth=".35" opacity=".65" /> : null; }) : null}</svg>
    ...
    {layers.incidents ? incidents.map((incident, index) => { const quay = quays.find((item) => item.id === incident.quayId); return quay ? <button key={incident.id} onClick={() => onSelectQuay(quay.id)} className={`absolute z-20 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rotate-45 border-2 border-white ${markerClass(incident.level)}`} style={{ left: `${quay.x + 2 + (index % 2)}%`, top: `${quay.y - 3}%` }} aria-label={incident.title} /> : null; }) : null}
...
export function ContextPanel({ title, subtitle, level, rows, actions, trend, empty = false }: { title: string; subtitle: string; level?: Level; rows: Array<[string, string]>; actions: Array<{ label: string; primary?: boolean; onClick: () => void }>; trend?: number[]; empty?: boolean }) {
