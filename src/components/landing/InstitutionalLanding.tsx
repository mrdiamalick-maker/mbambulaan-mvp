...
function PublicHeader() {
  return (
    <header className="relative z-50 border-b border-white/10 bg-[var(--mb-navy-900)] text-white">
      <div className="mx-auto flex h-16 max-w-[88rem] items-center justify-between gap-5 px-5 sm:px-8 lg:px-12">
        <Link href="/" className="flex min-w-0 items-center gap-3" aria-label="Accueil Mbàmbulaan">
          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-[3px] bg-[var(--mb-sand-300)] text-[11px] font-black text-[var(--mb-navy-900)]">Mb</span>
          <span className="min-w-0"><strong className="block text-[14px]">Mbàmbulaan</strong><span className="hidden text-[9px] font-bold uppercase tracking-[0.12em] text-white/45 sm:block">Coordination maritime nationale</span></span>
        </Link>
        <nav className="hidden items-center gap-7 text-[11px] font-semibold text-white/65 md:flex" aria-label="Navigation publique">
          <a href="#cycle" className="hover:text-white">Le cycle</a>
          <a href="#espaces" className="hover:text-white">Les espaces</a>
          <a href="#preuves" className="hover:text-white">Les preuves</a>
          <a href="#valeur" className="hover:text-white">Valeur institutionnelle</a>
        </nav>
        <Link href="/espace-prive" className="inline-flex h-9 shrink-0 items-center rounded-[3px] border border-white/20 px-3 text-[11px] font-bold text-white transition-colors hover:bg-white/10">Accès privé</Link>
      </div>
    </header>
  );
}
...
function Hero() {
  return (
    <section className="relative bg-[var(--mb-navy-900)] text-white">
      <div className="pointer-events-none absolute inset-0 opacity-30 [background-image:linear-gradient(rgba(76,147,176,.12)_1px,transparent_1px),linear-gradient(90deg,rgba(76,147,176,.08)_1px,transparent_1px)] [background-size:64px_64px]" />
      <div className="relative mx-auto grid max-w-[88rem] gap-12 px-5 pb-16 pt-14 sm:px-8 lg:grid-cols-[minmax(0,.84fr)_minmax(36rem,1.16fr)] lg:items-center lg:px-12 lg:pb-20 lg:pt-20">
        <div className="max-w-2xl">
          ...
          <h1 className="mt-5 text-[clamp(2.5rem,5vw,4.8rem)] font-semibold leading-[1.02] tracking-[-0.02em] text-[var(--mb-offwhite)]">La salle de contrôle numérique de la pêche artisanale sénégalaise</h1>
          <p className="mt-7 max-w-xl text-[15px] leading-7 text-white/66 sm:text-[16px]">Mbàmbulaan rend visible tout le cycle de la pêche : départs, activité en mer, retours, débarquements, besoins de la filière, financements et décisions institutionnelles.</p>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link href="/espace-prive/etat" className="inline-flex h-12 items-center justify-center gap-3 rounded-[3px] bg-[var(--mb-sand-300)] px-5 text-[12px] font-bold text-[var(--mb-navy-900)] transition-colors hover:bg-white">Découvrir la console <span aria-hidden="true">→</span></Link>
            <a href="#valeur" className="inline-flex h-12 items-center justify-center rounded-[3px] border border-white/20 px-5 text-[12px] font-bold text-white transition-colors hover:bg-white/10">Voir la promesse ministère</a>
          </div>
          <div className="mt-9 flex flex-wrap gap-x-6 gap-y-2 border-t border-white/10 pt-5 font-mono text-[9px] uppercase tracking-[0.08em] text-white/40"><span>Cycle suivi de bout en bout</span><span>Validation humaine</span><span>Documents transmissibles</span></div>
        </div>
...
function InstitutionalOutputs() {
  return (
    <section id="preuves" className="scroll-mt-16 bg-[var(--mb-navy-900)] text-white">
      <div className="mx-auto max-w-[88rem] px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
        <SectionIntroduction dark eyebrow="Des résultats tangibles" title="Ce que Mbàmbulaan produit" text="Des documents reliés aux informations terrain, aux validations humaines et aux décisions prises." />
...
function ValueChain() {
  return (
    <section className="border-y border-[var(--mb-neutral-200)] bg-[var(--mb-foam)]">
      <div className="mx-auto grid max-w-[88rem] gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(23rem,.7fr)] lg:items-center lg:px-12">
        <div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--mb-ocean-600)]">Valeur pour la filière</p><h2 className="mt-4 max-w-3xl text-[30px] font-semibold leading-tight text-[var(--mb-navy-900)]">Rendre visibles les besoins, les preuves d’impact et les opportunités de financement.</h2><p className="mt-5 max-w-2xl text-[12px] leading-6 text-[var(--mb-neutral-600)]">La coordination commence avec les acteurs et leurs réalités. Les informations qualifiées servent ensuite les programmes, les partenaires et les décisions publiques.</p></div>
        <div className="flex flex-wrap gap-2">{actors.map((actor) => <span key={actor} className="rounded-[2px] border border-[var(--mb-ocean-600)]/20 bg-white px-3 py-2 text-[10px] font-semibold text-[var(--mb-navy-700)]">{actor}</span>)}</div>
      </div>
    </section>
  );
}
