...
export function MinistryDossiersView({ dossiers, onOpenDossier }: { dossiers: DossierOperationnel[]; onOpenDossier: (dossier: DossierOperationnel) => void }) {
...
function FilterSelect({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) { return <label className="grid gap-1 text-[10px] font-bold text-[var(--mb-neutral-600)]">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="h-9 min-w-36 border border-[var(--mb-neutral-200)] bg-white px-2 text-[11px] font-semibold text-[var(--mb-navy-900)]">{options.map((option) => <option key={option}>{option}</option>)}</select></label>; }
