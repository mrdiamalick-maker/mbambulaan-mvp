...
export type NeedStatus = "Signalé" | "Qualifié" | "Éligible au financement" | "Financé";
...
export function FiliereNeedsView({ needs, opportunities, artifacts, onOpenWorkflow }: { needs: CommunityNeed[]; opportunities: FundingOpportunity[]; artifacts: GeneratedArtifact[]; onOpenWorkflow: (kind: "qualification" | "funding" | "partner", context: WorkflowContext) => void }) {
  ...
  const rows = useMemo<NeedViewRow[]>(() => needs.map((need) => {
    ...
    const currentStatus: NeedStatus = opportunity?.status === "Financé" ? "Financé" : opportunity?.status && opportunity.status !== "À qualifier" ? "Éligible au financement" : opportunity ? "Qualifié" : "Signalé";
  ...
  return <div className="grid min-w-0 gap-2 bg-[var(--mb-neutral-100)] p-2 xl:grid-cols-[minmax(0,1fr)_330px]">
    <section className="min-w-0 border border-[var(--mb-neutral-200)] bg-white"><header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--mb-neutral-200)] px-3 py-3"><div><h2 className="text-[14px] font-semibold text-[var(--mb-navy-900)]">Besoins de la filière</h2><p className="mt-1 text-[9px] text-[var(--mb-neutral-600)]">Du signal terrain au financement, une seule lecture et une prochaine action.</p></div><div className="flex flex-wrap gap-1">{(["Tous", "Signalé", "Qualifié", "Éligible au financement", "Financé"] as const).map((item) => <button key={item} onClick={() => setStatus(item)} className={`h-8 border px-2 text-[9px] font-bold ${status === item ? "border-[var(--mb-ocean-600)] bg-[var(--mb-ocean-600)] text-white" : "border-[var(--mb-neutral-200)] bg-white text-[var(--mb-neutral-600)]"}`}>{item}</button>)}</div></header>
      <div className="min-w-0 overflow-x-auto"><table className="w-full min-w-[760px] border-collapse text-left"><thead><tr className="h-9 bg-[var(--mb-offwhite)] font-mono text-[8px] uppercase text-[var(--mb-neutral-600)]">{["Besoin", "Territoire / communauté", "Montant", "Catégorie", "Acteurs", "Statut", "Prochaine action"].map((header) => <th key={header} className="border-b border-[var(--mb-neutral-200)] px-3">{header}</th>)}</tr></thead><tbody>{filtered.map((row) => <tr key={row.need.id} onClick={() => setSelectedId(row.need.id)} className={`cursor-pointer border-b border-[var(--mb-neutral-100)] text-[10px] hover:bg-[var(--mb-foam)] ${selected?.need.id === row.need.id ? "bg-[var(--mb-foam)]" : "bg-white"}`}><td className="px-3 py-3 font-semibold text-[var(--mb-navy-900)]">{row.opportunity?.title || row.need.need}</td><td className="px-3 py-3">{row.need.place}<span className="block text-[8px] text-[var(--mb-neutral-400)]">{row.need.actors}</span></td><td className="px-3 py-3 font-mono font-bold">{formatFcfa(row.opportunity?.estimatedAmount || 0)}</td><td className="px-3 py-3">{row.opportunity?.category || "À qualifier"}</td><td className="px-3 py-3 font-mono">{row.opportunity?.beneficiaries || "—"}</td><td className="px-3 py-3"><StatusBadge level={row.status === "Signalé" ? "surveillance" : "normal"}>{row.status}</StatusBadge></td><td className="px-3 py-3 font-semibold text-[var(--mb-ocean-600)]">{nextAction(row.status)}</td></tr>)}</tbody></table></div>
    </section>
    <aside className="grid content-start gap-2">{selected && context ? <section className="border border-[var(--mb-neutral-200)] bg-white"><header className="bg-[var(--mb-navy-900)] px-3 py-3 text-white"><p className="font-mono text-[8px] uppercase text-[var(--mb-ocean-400)]">Partenaires & preuves</p><h3 className="mt-1 text-[13px] font-semibold">{selected.opportunity?.title || selected.need.need}</h3><p className="mt-1 text-[9px] text-white/55">{selected.need.place} · {selected.status}</p></header><dl className="divide-y divide-[var(--mb-neutral-100)] px-3"><Fact label="Partenaire compatible" value={selected.opportunity?.compatibleFunder || "À identifier"} /><Fact label="Compatibilité" value={selected.opportunity ? `${selected.opportunity.compatibilityScore}%` : "À calculer"} /><Fact label="Impact attendu" value={selected.opportunity?.expectedImpact || "À documenter"} /><Fact label="Preuve disponible" value={artifacts.some((artifact) => artifact.scope.includes(selected.need.place)) ? "Document enregistré" : "Preuve terrain manquante"} /></dl><div className="grid gap-2 border-t border-[var(--mb-neutral-100)] p-3"><p className="text-[9px] leading-4 text-[var(--mb-neutral-600)]">{actionHelper(selected.status)}</p>{selected.status === "Signalé" ? <button onClick={() => onOpenWorkflow("qualification", context)} className={primaryButton}>Qualifier ce besoin</button> : <><button onClick={() => onOpenWorkflow("funding", context)} className={primaryButton}>Constituer le dossier de financement</button><button onClick={() => onOpenWorkflow("partner", context)} className={secondaryButton}>Solliciter le partenaire</button></>}</div></section> : null}
...
function buildContext(row: NeedViewRow): WorkflowContext { return { title: row.opportunity?.title || row.need.need, scope: row.need.place, sourceId: row.opportunity?.id || row.need.id, description: row.opportunity?.expectedImpact || row.need.nextAction, amount: String(row.opportunity?.estimatedAmount || 0), beneficiaries: String(row.opportunity?.beneficiaries || 0), partner: row.opportunity?.compatibleFunder }; }
function nextAction(status: NeedStatus) { return status === "Signalé" ? "Qualifier le besoin" : status === "Qualifié" ? "Vérifier l’éligibilité" : status === "Éligible au financement" ? "Constituer le dossier" : "Suivre l’impact"; }
function actionHelper(status: NeedStatus) { return status === "Signalé" ? "Qualifier ce besoin permettra d’en mesurer la maturité et de l’évaluer pour un financement." : "Le dossier pourra être transmis à un partenaire ou bailleur après validation humaine."; }
