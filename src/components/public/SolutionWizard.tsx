...
export function SolutionWizard({ initialIntent, initialCategory, initialTerritory, source = "web", context }: SolutionWizardProps) {
  ...
  {/* LOT 6 (mandat §11) : "qualifie maintenant votre besoin" pouvait se
      lire comme une qualification déjà faite — aucune qualification
      humaine n'a encore eu lieu au moment de la soumission. Reformulé
      pour dire exactement ce qui est vrai : la demande est enregistrée et
      ENTRE dans le processus de qualification, sans SLA promis. */}
  if (step === "done") return <div className="pub-card p-7 md:p-9"><span className="grid size-12 place-items-center rounded-full bg-[rgba(182,82,47,.10)] text-[var(--pub-turquoise-500)]"><CheckCircle2 /></span><h2 className="mt-5 text-2xl font-[740] tracking-[-.035em] text-[var(--pub-deep-900)]">Votre demande {reference} est enregistrée.</h2><p className="mt-3 text-sm leading-6 text-[var(--pub-stone-700)]">Elle entre maintenant dans le processus de qualification Mbàmbulaan. Nous conservons cette référence pour la suite et reprenons contact par {channel === "whatsapp" ? "WhatsApp" : channel === "telephone" ? "téléphone" : "e-mail"}.</p><div className="mt-6 flex flex-wrap gap-2"><Link href="/atlas" className="pub-btn pub-btn-primary">Explorer l’Atlas <ArrowRight size={15}/></Link><Link href="/decouvrir" className="pub-btn pub-btn-outline">Continuer à découvrir</Link></div></div>;
