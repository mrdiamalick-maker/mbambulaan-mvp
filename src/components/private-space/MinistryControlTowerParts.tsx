...
export function ModuleTabs({ active, onChange }: { active: "map" | "community" | "proof"; onChange: (module: "map" | "community" | "proof") => void }) {
  const items = [
    ["map", "Carte nationale", "Voir, détecter, qualifier"],
    ["community", "Communautés & impact", "Transformer le terrain en programmes"],
...
export function ControlMap({ quays, pirogues, zones, selectedId, onSelect }: { quays: Quay[]; pirogues: PilotPirogue[]; zones: SensitiveZone[]; selectedId: string; onSelect: (id: string) => void }) {
  return <div className="relative min-h-[34rem] overflow-hidden rounded-[2.4rem] border border-cyan-200 bg-[radial-gradient(circle_at_22%_25%,rgba(34,211,238,0.28),transparent_26%),radial-gradient(circle_at_70%_78%,rgba(16,185,129,0.18),transparent_24%),linear-gradient(145deg,#06263f_0%,#073854_42%,#0e746d_100%)] p-5 text-white shadow-2xl shadow-cyan-950/20">
    ...
    <div className="absolute left-5 top-5 rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur-md">
      ...
      <p className="mt-2 max-w-sm text-xs font-bold leading-5 text-cyan-50">La cartographie devient le point d'entrée de la coordination.</p>
    </div>
    <div className="absolute bottom-5 left-5 right-5 flex flex-wrap gap-2 text-xs font-black">
      ...
      <span className="rounded-full bg-white/12 px-3 py-2 backdrop-blur">Signaux à qualifier</span>
    </div>
  </div>;
}
...
export function QuayPanel({ quay, onCommunity, onProof }: { quay: Quay; onCommunity: () => void; onProof: () => void }) {
  return <ShellCard className="h-full">
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-cyan-700">Quai sélectionné</p><h3 className="mt-2 text-3xl font-black text-slate-950">{quay.name}</h3><p className="text-sm font-bold text-slate-500">{quay.region} · {quay.commune}</p></div><Badge tone={quay.tension}>{tensionLabels[quay.tension]}</Badge></div>
    <div className="mt-5 grid gap-3 sm:grid-cols-2"><DataPoint label="Activité" value={`${quay.volumeTons} t`} /><DataPoint label="Débarquements" value={String(quay.landings)} /><DataPoint label="Pirogues actives" value={String(quay.activePirogues)} /><DataPoint label="Alertes" value={String(quay.alerts)} /></div>
    <div className="mt-5"><p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">Espèces principales</p><div className="mt-2 flex flex-wrap gap-2">{quay.mainSpecies.map((species) => <span key={species} className="rounded-full bg-cyan-50 px-3 py-1 text-xs font-black text-cyan-900">{species}</span>)}</div></div>
    ...
    <div className="mt-5"><p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">Signaux à qualifier</p><ul className="mt-2 grid gap-2">{quay.signalsToQualify.map((signal) => <li key={signal} className="rounded-xl border border-cyan-100 bg-cyan-50/70 px-3 py-2 text-xs font-bold text-cyan-950">{signal}</li>)}</ul></div>
    <div className="mt-5 rounded-2xl bg-gradient-to-br from-cyan-700 to-teal-700 p-4 text-white"><p className="text-xs font-black uppercase tracking-[0.12em] text-cyan-100">Recommandation ministère</p><p className="mt-2 text-sm font-bold leading-6">{quay.ministryRecommendation}</p></div>
    <div className="mt-5 flex flex-wrap gap-2"><button onClick={onCommunity} className={secondaryButton}>Voir le besoin</button><button onClick={onProof} className={primaryButton}>Préparer preuve</button></div>
  </ShellCard>;
}

export function LiveFeed({ items }: { items: LiveSignal[] }) {
  return <ShellCard><p className="text-xs font-black uppercase tracking-[0.16em] text-cyan-700">Flux temps réel simulé</p><div className="mt-4 grid gap-3">{items.map((item) => <div key={item.id} className="grid grid-cols-[3.5rem_1fr_auto] items-center gap-3 rounded-2xl border border-cyan-100 bg-white/75 p-3"><span className="text-sm font-black text-cyan-800">{item.time}</span><div><p className="text-sm font-black text-slate-900">{item.label}</p><p className="text-xs font-semibold text-slate-500">{item.territory}</p></div><Badge tone={item.level}>{item.level}</Badge></div>)}</div></ShellCard>;
}

export function AssistedPanel({ insight }: { insight: AssistedInsight }) {
  return <ShellCard className="bg-gradient-to-br from-white via-cyan-50/75 to-emerald-50/70"><div className="flex flex-wrap items-center gap-2"><Badge tone="info">Lecture assistée</Badge><span className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">V1 simulée</span></div><h3 className="mt-3 text-xl font-black text-slate-950">{insight.title}</h3><p className="mt-3 text-sm font-bold leading-6 text-slate-600">{insight.body}</p><p className="mt-4 rounded-2xl bg-white/85 p-3 text-sm font-black leading-6 text-cyan-950">Proposition à valider : {insight.recommendation}</p><p className="mt-3 text-xs font-bold text-slate-500">En V1, Mbàmbulaan préfigure une lecture assistée. L'humain valide chaque décision.</p></ShellCard>;
}

export function NeedCard({ need }: { need: CommunityNeed }) {
  return <article className="rounded-[1.6rem] border border-cyan-100 bg-white/88 p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.12em] text-cyan-700">{need.category}</p><h3 className="mt-2 text-lg font-black text-slate-950">{need.territory}</h3></div><Badge tone={need.urgency}>{tensionLabels[need.urgency]}</Badge></div><p className="mt-3 text-sm font-bold leading-6 text-slate-600">{need.summary}</p><div className="mt-4 grid gap-2 text-xs font-bold text-slate-600"><DataRow label="Public" value={need.audience} /><DataRow label="Bénéficiaires" value={`${need.beneficiaries} estimés`} /><DataRow label="Statut" value={need.status} /><DataRow label="Partenaire" value={need.partner} /></div><div className="mt-4 flex flex-wrap gap-2"><button className={primaryButton}>Qualifier le besoin</button><button className={secondaryButton}>Transformer en projet</button><button className={secondaryButton}>Préparer note</button></div></article>;
}

export function ProjectCard({ project }: { project: ImpactProject }) {
  return <article className="rounded-[1.8rem] border border-cyan-100 bg-gradient-to-br from-white via-cyan-50/45 to-emerald-50/35 p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><h3 className="text-xl font-black text-slate-950">{project.name}</h3><Badge tone={project.urgency}>{tensionLabels[project.urgency]}</Badge></div><p className="mt-2 text-sm font-bold text-slate-500">{project.territory} · {project.owner}</p><p className="mt-4 text-sm font-bold leading-6 text-slate-700">{project.problem}</p><div className="mt-4 grid gap-2 text-xs font-bold text-slate-600"><DataRow label="Impact attendu" value={project.expectedImpact} /><DataRow label="Budget" value={project.budget} /><DataRow label="Statut" value={project.status} /><DataRow label="Partenaire cible" value={project.partnerTarget} /><DataRow label="Maturité" value={project.maturity} /></div><div className="mt-5 flex flex-wrap gap-2"><button className={primaryButton}>Préparer fiche partenaire</button><button className={secondaryButton}>Classer prioritaire</button><button className={secondaryButton}>Demander complément</button></div></article>;
}

export function ProgramCard({ program }: { program: TrainingProgram }) {
  return <article className="rounded-[1.6rem] border border-cyan-100 bg-white/88 p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-black text-slate-950">{program.title}</h3><Badge tone={program.status.includes("Prioritaire") ? "attention" : "info"}>{program.status}</Badge></div><p className="mt-3 text-sm font-bold leading-6 text-slate-600">{program.objective}</p><div className="mt-4 grid gap-2 text-xs font-bold text-slate-600"><DataRow label="Territoire" value={program.territory} /><DataRow label="Public" value={program.audience} /><DataRow label="Partenaire" value={program.partner} /><DataRow label="Période" value={program.period} /><DataRow label="Participants" value={`${program.participants} attendus`} /><DataRow label="Impact" value={program.impactIndicator} /></div><div className="mt-4 flex flex-wrap gap-2"><button className={primaryButton}>Planifier session</button><button className={secondaryButton}>Préparer note partenaire</button><button className={secondaryButton}>Suivre participation</button></div></article>;
}

export function PreventionSignalCard({ signal }: { signal: PreventionSignal }) {
  return <article className="rounded-[1.4rem] border border-cyan-100 bg-white/88 p-4"><div className="flex items-start justify-between gap-3"><h3 className="text-base font-black text-slate-950">{signal.label}</h3><Badge tone={signal.level}>{signal.level}</Badge></div><p className="mt-2 text-xs font-bold uppercase tracking-[0.1em] text-cyan-700">{signal.territory} · {signal.type}</p><p className="mt-3 text-sm font-bold leading-6 text-slate-600">{signal.recommendedAction}</p><p className="mt-3 text-xs font-semibold text-slate-500">Source mockée : {signal.source}. Situation à qualifier, sans accusation automatique.</p></article>;
}
