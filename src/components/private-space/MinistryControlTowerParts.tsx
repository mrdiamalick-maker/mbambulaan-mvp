...
export function DataTrustBadge({ level, compact = false, source, onInspect }: { level: DataTrustLevel; compact?: boolean; source?: string; onInspect?: () => void }) {
  const detail = trustDetails[level];
  const className = `inline-flex items-center gap-1.5 rounded-[3px] border px-2 py-1 font-mono text-[8px] font-bold uppercase tracking-[0.05em] ${detail.className}`;
  const content = <><span className="h-1.5 w-1.5 rounded-full bg-current" />{detail.label}{compact ? null : <span className="hidden normal-case tracking-normal opacity-75 2xl:inline">· {source || detail.helper}</span>}</>;
  return onInspect ? <button type="button" onClick={onInspect} title={`${detail.helper}. ${source || "Consulter la preuve ou l’historique associé."}`} className={className}>{content}<span aria-hidden="true">↗</span></button> : <span title={`${detail.helper}. ${source || "Source locale de démonstration, sans serveur."}`} className={className}>{content}</span>;
}
...
export function ContextPanel({ title, subtitle, level, trustLevel, rows, actions, journey, trend, pirogue, referents, empty = false }: { title: string; subtitle: string; level?: Level; trustLevel?: DataTrustLevel; rows: Array<[string, string]>; actions: ContextAction[]; journey: RecommendedJourney; trend?: number[]; pirogue?: Pirogue | null; referents?: ReactNode; empty?: boolean }) {
  ...
  return <aside className="h-full min-w-0 overflow-y-auto border-l border-[var(--mb-neutral-200)] bg-white"><div className="border-b border-[var(--mb-neutral-100)] bg-[var(--mb-navy-900)] px-4 py-3 text-white"><div className="flex items-start justify-between gap-2"><div><p className="font-mono text-[9px] text-[var(--mb-ocean-400)]">{journey.entityType.toUpperCase()} SÉLECTIONNÉ</p><h2 className="mt-1 text-[15px] font-semibold">{title}</h2><p className="mt-1 text-[10px] text-white/60">{subtitle}</p></div>{level ? <StatusBadge level={level} /> : null}</div>{trustLevel ? <div className="mt-3 flex items-center gap-2"><DataTrustBadge level={trustLevel} source={journey.trustHelper} onInspect={() => document.getElementById("atlas-evidence")?.scrollIntoView({ behavior: "smooth", block: "start" })} /><span className="text-[8px] leading-3 text-white/45">Cliquez pour voir l’historique local.</span></div> : null}{trend?.length ? <div className="mt-3"><p className="font-mono text-[8px] text-white/40">ACTIVITÉ · 7 JOURS</p><div className="mt-1 flex h-7 items-end gap-1">{trend.map((value, index) => <span key={index} className="flex-1 bg-[var(--mb-ocean-400)]/75" style={{ height: `${Math.max(18, value)}%` }} />)}</div></div> : null}</div>{pirogue ? <CycleTimeline pirogue={pirogue} /> : null}<section className="border-b border-[var(--mb-neutral-200)] bg-[var(--mb-foam)] p-3"><p className="font-mono text-[8px] font-bold uppercase tracking-[0.09em] text-[var(--mb-ocean-600)]">Que faire maintenant ?</p><h3 className="mt-1 text-[13px] font-semibold text-[var(--mb-navy-900)]">{recommended?.label}</h3><p className="mt-1 text-[10px] leading-4 text-[var(--mb-neutral-600)]">{journey.reason}</p>{recommended ? <button onClick={recommended.onClick} className={`${primaryButton} mt-3 w-full`}>{recommended.label}</button> : null}<p className="mt-2 text-[9px] leading-4 text-[var(--mb-neutral-600)]"><strong className="text-[var(--mb-navy-900)]">Après cette action :</strong> {journey.expectedResult}</p></section><section className="grid gap-3 border-b border-[var(--mb-neutral-200)] p-3"><JourneySection title="Situation actuelle"><p>{journey.situation}</p><p className="mt-1 font-semibold text-[var(--mb-navy-900)]">{journey.businessStatus}</p></JourneySection><JourneySection title="Ce que nous savons"><p>{journey.known}</p></JourneySection><JourneySection title="Déjà fait"><CheckedList items={journey.alreadyDone} /></JourneySection>{journey.waitingFor ? <JourneySection title="En attente"><p>{journey.waitingFor}</p></JourneySection> : null}{journey.blockingPoint ? <JourneySection title="Point bloquant" tone="warning"><p>{journey.blockingPoint}</p></JourneySection> : null}<JourneySection title="Étape suivante"><p>{journey.nextStep}</p></JourneySection></section><details className="border-b border-[var(--mb-neutral-200)]"><summary className="cursor-pointer px-4 py-3 text-[10px] font-bold text-[var(--mb-navy-900)]">Voir les informations détaillées</summary><dl className="divide-y divide-[var(--mb-neutral-100)] px-4">{rows.map(([label, value]) => <div key={label} className="grid min-h-9 grid-cols-[6.5rem_minmax(0,1fr)] items-center gap-2 text-[11px]"><dt className="text-[var(--mb-neutral-600)]">{label}</dt><dd className="text-right font-semibold text-[var(--mb-neutral-900)]">{value}</dd></div>)}</dl>{referents}</details><ActionGroup title="Autres actions utiles" actions={secondary} /><ActionGroup title="Consulter ou transmettre" actions={consultation} /></aside>;
}

function JourneySection({ title, children, tone = "default" }: { title: string; children: ReactNode; tone?: "default" | "warning" }) {
  return <div className={tone === "warning" ? "border-l-2 border-[var(--mb-amber-500)] bg-[var(--mb-amber-500)]/5 px-2 py-1.5" : ""}><h3 className="font-mono text-[8px] font-bold uppercase tracking-[0.07em] text-[var(--mb-neutral-400)]">{title}</h3><div className="mt-1 text-[9px] leading-4 text-[var(--mb-neutral-600)]">{children}</div></div>;
}

function CheckedList({ items }: { items: string[] }) {
  return items.length ? <ul className="grid gap-1">{items.map((item) => <li key={item} className="flex gap-2"><span className="font-bold text-[var(--mb-green-600)]">✓</span><span>{item}</span></li>)}</ul> : <p>Aucune action enregistrée pour le moment.</p>;
}
