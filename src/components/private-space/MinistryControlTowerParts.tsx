...
export function ContextPanel({ title, subtitle, level, rows, actions, trend, pirogue, empty = false }: { title: string; subtitle: string; level?: Level; rows: Array<[string, string]>; actions: Array<{ label: string; helper?: string; primary?: boolean; onClick: () => void }>; trend?: number[]; pirogue?: Pirogue | null; empty?: boolean }) {
  ...
  return <aside className="h-full min-w-0 border-l border-[var(--mb-neutral-200)] bg-white"><div className="border-b border-[var(--mb-neutral-100)] bg-[var(--mb-navy-900)] px-4 py-3 text-white"><div className="flex items-start justify-between gap-2"><div><p className="font-mono text-[9px] text-[var(--mb-ocean-400)]">DOSSIER SÉLECTIONNÉ</p><h2 className="mt-1 text-[15px] font-semibold">{title}</h2><p className="mt-1 text-[10px] text-white/60">{subtitle}</p></div>{level ? <StatusBadge level={level} /> : null}</div>{trend?.length ? <div className="mt-3"><p className="font-mono text-[8px] text-white/40">ACTIVITÉ · 7 JOURS</p><div className="mt-1 flex h-7 items-end gap-1">{trend.map((value, index) => <span key={index} className="flex-1 bg-[var(--mb-ocean-400)]/75" style={{ height: `${Math.max(18, value)}%` }} />)}</div></div> : null}</div>{pirogue ? <CycleTimeline pirogue={pirogue} /> : null}<dl className="divide-y divide-[var(--mb-neutral-100)] px-4">{rows.map(([label, value]) => <div key={label} className="grid min-h-9 grid-cols-[6.5rem_minmax(0,1fr)] items-center gap-2 text-[11px]"><dt className="text-[var(--mb-neutral-600)]">{label}</dt><dd className="text-right font-semibold text-[var(--mb-neutral-900)]">{value}</dd></div>)}</dl><div className="grid gap-2 border-t border-[var(--mb-neutral-100)] p-3">{actions.map((action) => <div key={action.label} className="grid gap-1">{action.helper ? <p className="text-[9px] leading-3 text-[var(--mb-neutral-600)]">{action.helper}</p> : null}<button onClick={action.onClick} className={action.primary ? primaryButton : secondaryButton}>{action.label}</button></div>)}</div></aside>;
}
