...
export default function PublicCommunityPage() {
  ...
  return <main className="min-h-screen bg-[#f4f7f6]">
    <PublicHeader dark />
    <section className="overflow-hidden bg-[#031a22] px-5 py-16 text-white md:px-10 md:py-20"><div className="mx-auto grid max-w-[1500px] gap-10 lg:grid-cols-[1fr_.7fr] lg:items-end"><div><p className="label-inverse">Réseau & intelligence collective</p><h1 className="mt-4 max-w-4xl text-4xl font-[760] leading-[1.02] tracking-[-.05em] md:text-6xl">Le terrain parle. La filière apprend. Les partenaires peuvent agir.</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-white/66">Besoins, capacités, bonnes pratiques et opportunités se rejoignent dans un espace public vivant, puis deviennent des actions suivies lorsqu’ils sont qualifiés.</p></div><div className="grid grid-cols-3 gap-px overflow-hidden rounded-2xl bg-white/15">{[[state.communityPosts.length,"contributions"],[state.territories.length,"territoires"],[`${(funded/1_000_000).toFixed(0)} M`,"FCFA documentés"]].map(([value,label])=><div key={String(label)} className="bg-white/[.06] p-4"><p className="text-2xl font-black">{value}</p><p className="mt-1 text-xs text-[#a9c9cd]">{label}</p></div>)}</div></div></section>
