...
describe("DevelopmentProgramRuntime", () => {
  it("refuse une allocation supérieure au plafond", () => {
    ...
    expect(() => runtime.execute({ commandId: "budget", actorId: "government-01", activeTerritoryId: "territory-national", command: {
      type: "add_budget_line", programId: "program-01", at, budgetLine: {
  ...
  it("exécute un programme jusqu'à l'évaluation d'impact", () => {
    const runtime = new DevelopmentProgramRuntime();
    ...
    expect(result.snapshot.commandCenters[0].servedBeneficiaryCount).toBe(1);
    expect(result.snapshot.commandCenters[0].budgetExecutionPercent).toBe(50);
    expect(result.snapshot.commandCenters[0].latestValueCreatedAndProtectedXof).toBe(45_000_000);
  });
});
