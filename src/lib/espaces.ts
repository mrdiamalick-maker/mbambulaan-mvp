...
import { getActorProfile } from "@/lib/reference";
...
export const espaces: EspaceProfile[] = [
  {
    slug: "pecheur",
    role: "Pêcheur",
    mission: getMission("Pêcheur", "Déclarer ses arrivages, suivre leur statut et voir les réservations issues de son activité."),
    needs: getNeeds("Pêcheur", ["Publier rapidement un lot", "Consulter ses declarations", "Voir les reservations", "Suivre les transactions"]),
    features: [
  ...
  },
  {
    slug: "mareyeur",
    role: "Mareyeur",
    mission: getMission("Mareyeur", "Rechercher des arrivages, publier des besoins et réserver les lots compatibles."),
    needs: getNeeds("Mareyeur", ["Trouver du volume disponible", "Publier une demande", "Comparer les opportunites", "Sécuriser une transaction"]),
    features: [
  ...
  },
  {
    slug: "transformateur",
    role: "Transformateur",
    mission: getMission("Transformateur", "Planifier les besoins industriels, réserver des lots et suivre les livraisons."),
    needs: getNeeds("Transformateur", ["Exprimer un besoin industriel", "Réserver un lot compatible", "Suivre les livraisons", "Recevoir les alertes utiles"]),
    features: [
      { label: "Publier un besoin", href: "/besoins" },
  ...
  },
  {
    slug: "collectivite",
    role: "Collectivité",
    mission: getMission("Collectivité", "Observer la filière, comprendre l'activité des quais et suivre les signaux territoriaux."),
    needs: getNeeds("Collectivité", ["Voir les indicateurs", "Comparer les quais", "Identifier les alertes", "Lire l'impact territorial"]),
    features: [
  ...
  },
  {
    slug: "admin",
    role: "Administrateur",
    mission: getMission("Administration", "Accéder à la vue globale de coordination et piloter tous les modules du MVP."),
    needs: getNeeds("Administration", ["Superviser les données mockées", "Contrôler les flux", "Voir les opportunités", "Suivre l'activité complète"]),
    features: [
...
function getMission(profil: string, fallback: string) {
  return getActorProfile(profil)?.description ?? fallback;
}

function getNeeds(profil: string, fallback: string[]) {
  return getActorProfile(profil)?.responsabilites ?? fallback;
}
