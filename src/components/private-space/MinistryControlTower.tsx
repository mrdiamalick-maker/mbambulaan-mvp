...
function AtlasMaritime({ scope, setScope, evidence, artifacts, alerts, verifiedIds, openWorkflow }: {
  ...
  return <section className="min-h-full">
    <WorkspaceHeader title="Atlas maritime" question="Surveiller, vérifier et agir sur les signaux du littoral et de la mer." scope={scope} onScopeChange={(next) => { setScope(next as Scope); setQuayFilter("Tous"); setSelection(null); }} onExport={() => openWorkflow("export-zone", { title: "Relevé de zone maritime", scope, description: "Quais, pirogues, débarquements, alertes et incidents du périmètre actif." })} />
    <FilterStrip>
    ...
    <div className="grid min-h-[calc(100vh-201px)] min-w-0 xl:grid-cols-[minmax(0,1fr)_340px] xl:grid-rows-[minmax(430px,1fr)_170px]">
      ...
      <div className="min-h-0 xl:row-span-2"><ContextPanel empty={!selection || !selectedQuay} title={selectedTitle} subtitle={verified ? "Vérification humaine enregistrée" : selectedBoat ? selectedBoat.status : selectedQuay ? `Dernier signal · ${selectedQuay.lastUpdated}` : ""} level={verified ? "normal" : selectedBoat?.level ?? selectedQuay?.level} trend={selectedQuay ? quayTrends[selectedQuay.id] : undefined} rows={contextRows} actions={[
        { label: "Vérifier", primary: true, onClick: () => openWorkflow("verification", selectedContext) },
        { label: "Créer une alerte", onClick: () => openWorkflow("alert", selectedContext) },
        { label: "Ouvrir la fiche complète", onClick: () => openWorkflow("full-record", selectedContext) },
        { label: "Exporter la zone", onClick: () => openWorkflow("export-zone", selectedContext) },
      ]} /></div>
...
function FiliereProgrammes({ scope, setScope, evidence, artifacts, opportunities, fundingRequests, qualifiedNeeds, partnerSolicitations, openWorkflow }: {
  ...
  return <section className="min-h-full">
    <WorkspaceHeader title="Filière & programmes" question="Transformer les besoins terrain en opportunités finançables et preuves d’impact." scope={scope} onScopeChange={(value) => setScope(value as Scope)} onExport={() => openWorkflow("institutional-export", { title: "Registre des opportunités de financement", scope, description: `${readyCount} opportunités prêtes à instruire pour ${formatFcfa(totalPotential)}.` })} />
    <MetricRow metrics={[
...
function PilotageInstitutionnel({ scope, setScope, evidence, artifacts, opportunities, fundingRequests, alerts, record, openWorkflow }: {
  ...
  const synthesis = `Aujourd’hui : ${scopedQuays.length} quais actifs, ${scopedAlerts.length} alertes ouvertes, 2 vérifications en retard et ${readyFunding.length} opportunités de financement prêtes à instruire.`;
  ...
  const decisions = [
    ...
    ...readyFunding.slice(0, 2).map((item) => ({ id: item.id, title: `Instruire ${item.title}`, detail: `${formatFcfa(item.estimatedAmount)} · ${item.compatibleFunder} · maturité ${item.maturityScore}%`, level: "normal" as Level, action: "Créer le dossier", onAction: () => openWorkflow("funding", { title: item.title, scope: item.territory, sourceId: item.id, description: item.expectedImpact, amount: String(item.estimatedAmount), beneficiaries: String(item.beneficiaries), partner: item.compatibleFunder }) })),
  ].slice(0, 4);

  return <section className="min-h-full">
    <WorkspaceHeader title="Pilotage institutionnel" question="Décider en vingt secondes, instruire et transmettre avec une preuve." scope={scope} onScopeChange={(value) => setScope(value as Scope)} onExport={() => openWorkflow("institutional-export", { title: "Export de pilotage institutionnel", scope, description: synthesis })} />
    <section className="flex flex-col gap-3 border-b border-[var(--mb-neutral-200)] bg-[var(--mb-navy-900)] px-4 py-4 text-white sm:flex-row sm:items-center sm:justify-between"><div><p className="font-mono text-[9px] uppercase tracking-[0.1em] text-[var(--mb-ocean-400)]">Synthèse de décision</p><p className="mt-2 max-w-5xl text-[15px] font-semibold leading-6">{synthesis}</p></div><button onClick={() => openWorkflow("note", noteContext)} className="h-9 shrink-0 rounded-[3px] bg-white px-3 text-[11px] font-bold text-[var(--mb-navy-900)]">Préparer une note institutionnelle</button></section>
    <MetricRow metrics={[
    ...
    <div className="grid min-w-0 gap-2 bg-[var(--mb-neutral-100)] p-2 xl:grid-cols-[minmax(0,1.25fr)_minmax(20rem,.75fr)]">
      <div className="grid min-w-0 gap-2">
        ...
        <section className="border border-[var(--mb-neutral-200)] bg-white"><div className="border-b border-[var(--mb-neutral-200)] px-3 py-2"><h3 className="text-[11px] font-bold">Opportunités à instruire</h3></div><DataTable headers={["Opportunité", "Territoire", "Bailleur", "Montant", "Maturité", "Décision"]} rows={readyFunding.map((item) => ({ id: item.id, cells: [<strong key="title">{item.title}</strong>, item.territory, item.compatibleFunder, <span key="amount" className="font-mono">{formatFcfa(item.estimatedAmount)}</span>, <span key="score" className="font-mono">{item.maturityScore}%</span>, <button key="action" onClick={() => openWorkflow("funding", { title: item.title, scope: item.territory, sourceId: item.id, description: item.expectedImpact, amount: String(item.estimatedAmount), beneficiaries: String(item.beneficiaries), partner: item.compatibleFunder })} className="font-bold text-[var(--mb-ocean-600)]">Instruire</button>] }))} /></section>
