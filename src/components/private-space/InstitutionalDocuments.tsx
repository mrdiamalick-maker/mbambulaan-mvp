...
export function DocumentPreview({ document, compact = false }: { document: GeneratedDocument; compact?: boolean }) {
  return <article className="overflow-hidden border border-[var(--mb-neutral-200)] bg-white">
    ...
    <div className={compact ? "grid gap-2 p-3" : "grid gap-4 p-4"}>
      {document.sections.map((section) => <DocumentSection key={section.title} section={section} compact={compact} />)}
...
function DocumentMetaGrid({ document }: { document: GeneratedDocument }) {
  return <dl className="grid grid-cols-2 border-b border-[var(--mb-neutral-200)] bg-[var(--mb-offwhite)] sm:grid-cols-4">{[["Périmètre", document.perimeter], ["Date", document.createdAt], ["Validateur", document.validator], ["Source", "Mbàmbulaan · mock"]].map(([label, value]) => <div key={label} className="min-w-0 border-r border-[var(--mb-neutral-200)] px-3 py-2 last:border-r-0"><dt className="font-mono text-[8px] uppercase text-[var(--mb-neutral-400)]">{label}</dt><dd className="mt-1 truncate text-[9px] font-semibold text-[var(--mb-neutral-900)]">{value}</dd></div>)}</dl>;
}

function DocumentSection({ section, compact }: { section: DocumentSectionData; compact: boolean }) {
  return <section><h4 className="border-b border-[var(--mb-ocean-600)]/25 pb-1.5 text-[10px] font-bold text-[var(--mb-navy-900)]">{section.title}</h4><dl className={compact ? "mt-1" : "mt-2"}>{section.items.map((item) => <div key={item.label} className="grid grid-cols-[7rem_minmax(0,1fr)] gap-2 border-b border-[var(--mb-neutral-100)] py-1.5 text-[9px]"><dt className="text-[var(--mb-neutral-600)]">{item.label}</dt><dd className="font-semibold leading-4 text-[var(--mb-neutral-900)]">{item.value}</dd></div>)}</dl></section>;
}
