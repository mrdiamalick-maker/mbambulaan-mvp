...
describe("UnifiedWorkOrchestrationRuntime", () => {
  it("planifie affectation, rappel et escalade sans doublon", () => {
    ...
    expect(snapshot.notifications.filter((item) => item.notificationType === "reminder")).toHaveLength(1);
    expect(snapshot.notifications.filter((item) => item.notificationType === "escalation")).toHaveLength(1);
  });

  it("annule les notifications en attente quand la tâche est terminée", () => {
    const runtime = new UnifiedWorkOrchestrationRuntime();
    ...
    expect(snapshot.notifications.some((item) => item.notificationType === "completion")).toBe(true);
  });
});
