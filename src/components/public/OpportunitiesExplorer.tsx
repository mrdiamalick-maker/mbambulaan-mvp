        ...
        {filtered.map((item) => {
          ...
          return (
            <Link key={item.id} href={`/opportunites/${item.id}`} className="pub-card group flex min-h-72 flex-col p-5 transition hover:-translate-y-0.5 hover:border-[var(--pub-turquoise-500)]">
              <div className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[.11em] text-[var(--pub-turquoise-500)]"><Icon size={14}/>{item.type}</span><span className="text-[10px] font-bold text-[var(--pub-stone-500)]">Démonstration</span></div>
              ...
              <div className="mt-5 space-y-2 rounded-xl bg-[var(--pub-ivory-100)] p-4 text-xs text-[var(--pub-stone-700)]">
